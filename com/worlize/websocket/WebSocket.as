package com.worlize.websocket
{
   ...
   public class WebSocket extends EventDispatcher
   {
      ...
      private function fragmentAndSend(param1:WebSocketFrame) : void {
         ...
         {
            var _loc2_ = this.config.fragmentationThreshold;
      ...
      private function sendHandshake() : void {
         ...
         _loc2_ = _loc2_ + ("Cookie: " + this._cookie + "\r\n");
         if(this._origin)
         {
         ...
         if(this._protocols)
         {
            ...
            _loc2_ = _loc2_ + ("Sec-WebSocket-Protocol: " + _loc3_ + "\r\n");
         }
         _loc2_ = _loc2_ + "\r\n";
         if(this.debug)
         {
      ...
      private function readServerHandshake() : void {
         ...
         while(headersTerminatorIndex === -1 && (this.readHandshakeLine()))
         {
            ...
            headersTerminatorIndex = this.serverHandshakeResponse.search(new RegExp("\\r?\\n\\r?\\n"));
         }
         ...
         var lines:Array = this.serverHandshakeResponse.split(new RegExp("\\r?\\n"));
         responseLine = lines.shift();
         var responseLineMatch:Array = responseLine.match(new RegExp("^(HTTP\\/\\d\\.\\d) (\\d{3}) ?(.*)$","i"));
         ...
         try
         {
            while(lines.length > 0)
            {
               ...
               {
                  expectedKey = SHA1.hashToBase64(this.base64nonce + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11");
                  if(this.debug)
                  {
